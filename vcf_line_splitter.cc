...
class linepeeker {
  ...
  public:
    linepeeker(istream &input) : _input(input), _full(false), _count(0), _last_size(0) {
...
DEFINE_string(range, "",
              "chr:beg-end; include only lines with CHROM:POS within this inclusive range");
...
void writer_thread(const string &dest_prefix, const int part_num, const string &header,
                   unique_ptr<vector<string>> buf) {
    ...
    if (!ogz || bgzf_write(ogz, header.c_str(), header.size()) != header.size() ||
        bgzf_flush(ogz)) {
        throw runtime_error("Error opening part file " + path +
                            " for writing; delete if it already exists");
    }
    sz += header.size();

    for (auto it = buf->begin(); it != buf->end(); it++) {
        if (bgzf_write(ogz, it->c_str(), it->size()) != it->size() ||
            bgzf_write(ogz, "\n", 1) != 1) {
...

    {
        ...
        if (!FLAGS_quiet) {
            cerr << "vcf_line_splitter wrote " << path << " (" << sz / 1048576
                 << " MB before compression)" << endl;
        }
        --threads_active;
        ++threads_completed;
        write_s +=
...
bool part_finished(int lines, size_t bytes) {
    return (FLAGS_MB > 0) ? (bytes >= size_t(FLAGS_MB) * 1048576) : (lines >= FLAGS_lines);
}

bool in_range(const string &line, string range_chrom, unsigned long long range_beg,
...
bool part(const string &dest_prefix, const int part_num, const string &header, linepeeker &input,
    ...
    if (lines) {
        ...
        ++threads_active;
        stall_s +=
            chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - t1).count() /
            1000.0;
    }
...
Usage: bgzip -dc@ 4 | vcf_line_splitter -threads $(nproc) /destination/path/prefix");
    ...
    if (!FLAGS_quiet) {
        cerr << "vcf_line_splitter wrote " << part_num << " parts with " << records_read
             << " records and " << bytes_processed << " uncompressed bytes";
        if (records_skipped) {
            cerr << " (" << records_skipped << " records range-skipped)";
        }
        cerr << "; spent " << (int)read_s << "s reading and " << (int)write_s << "s writing"
             << ", " << (int)stall_s << "s stalled" << endl;
    }
